...
class ReportController extends ControllerBase {
  ...
  public function report() {
    $content = [];
    $content['#message'] = [
      '#markup' => $this->t('Below is a list of all Event RSVPs including the username, email address and the name of the event they will be attending.')
    ];
    $headers = [
      t('Name'),
      t('Event'),
    ...
    foreach($entries = $this->load() as $entry) {
      // Sanitize each entry, here checks that each $entry from DB is of plain text
